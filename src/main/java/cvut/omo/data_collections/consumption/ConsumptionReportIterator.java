...
public class ConsumptionReportIterator implements Iterator {
    ...
    public String next() {
        ...
        if (!curr.isNotConsume()) {
            ...
            res.append("[").append(currIdx + 1).append("] ").append(curr.getClass().getSimpleName() + " in " + curr.getRoom().getRoomName() + " on the " + curr.getRoom().getFloor().getNumberOfFloor() + " floor" + " has consumed ");
    ...
    private void generateRow(List<ConsumptionData> consumptionData, StringBuilder res, SourceType sourceType, char currency) {
        double sumStub = consumptionData.stream().filter(x -> x.getSourceType().equals(sourceType)).mapToDouble(ConsumptionData::getAmountOfConsumed).sum();

        double allConsumed = sourceTypes.get(sourceType) + sumStub;
        sourceTypes.put(sourceType, allConsumed);

        res.append(sourceType.name()).append(": ").append(sumStub).append(sourceType.getUNIT_OF_MEASUREMENT()).append(" (SUM = ").append(sourceType.getPRICE_FOR_UNIT() * sumStub).append(currency).append(") ");
    }
    ...
    private void addInfoAboutConsumptionFromAllTime(StringBuilder res) {
        ...
        for (java.util.Iterator<SourceType> it = sourceTypes.keys().asIterator(); it.hasNext(); ) {
            ...
            if (!sourceType.equals(SourceType.NOT_CONSUME)) {
                ...
                res.append(Constants.STARS).append(sum).append(sourceType.getUNIT_OF_MEASUREMENT()).append(" of ").append(sourceType).append(" was consumed during the whole time.").append(" (SUM = ").append(sourceType.getPRICE_FOR_UNIT() * sum).append(Constants.CURRENCY).append(") ");
            }
        }
